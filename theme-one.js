...
;(function ($) {
    ...
    $.fn.neonTheme = function (options) {
        ...
        /**
         * default_carrossel_home()
         */
        if (neon.default_carrossel_home !== false) {
            $j(document)
                .ready(function (e) {
                    default_carrossel_home()
                });
        ...
        if (neon.m_search !== false) {
            ...
            // observers para abrir e fechar a busca
                .on('click', '.header .search__button', function (e) {
                    ...
                    if (!$j(this).closest('.search').hasClass('on')) {
...
/**
 * Ativa o Carrossel da home
 */
function default_carrossel_home() {
    var Home__Banner__produtos = $j('#home__banner__produtos .products__list');

    if (Home__Banner__produtos.length) {
        Home__Banner__produtos.each(function (i, el) {
            $j(el).owlCarousel({
                singleItem: true,
                itemScaleUp: true,
                addClassActive : true,
    ...
    if (tabs.length > 0) {
        ...
        tabs.on('click', '.tabs__tab', function () {
            if (!$j(this).hasClass('on')) {
                ...
                $tab_name = $j(this).data("name");
                $j(this).addClass('on');
                $j('dd[data-name='+$tab_name+']').addClass('on');
            }
        });
    }
}
...
$j.fn.neonTheme.custom = {
    ...
    default_carrossel_home: true, //carrousel padrão dos produtos na home var id:home__banner__produtos
...
$j(document)
    .ready(function () {
        // document.ready
    	$j('input').blur();
        $j("#email_conta_review").blur();
        $j("#email_header__suaconta").blur(); 
    
        // Click scroll up
        $j(".footer__up").on("click" ,function(){
            $j("html, body").animate({
                scrollTop: 0
            }, 2000);  
        });
        ...
        // Product gallery
        if( $j('.product-image-gallery img') ){
            makeGallery();
            
            // Remove disabled
            $j('.more-views .product-image-thumbs').on( 'scroll.flickity', function( event, progress ) {
                $j('.flickity-button').removeAttr('disabled');
            });
        }  
    
    	//Adiciona marcas ao menu mobile
    	var marcas__mobile = '<li class="li--0 marcas"><div class="header-mobile__list"> <div class="header-mobile__list-sep"> <a href="https://logitechstore.com.br/core"> <img src="https://www.logitechstore.com.br/media/wysiwyg/Base/svg/minilogo-logitech.svg" alt=""/> </a> </div><div class="header-mobile__list-sep"> <a href="https://logitechstore.com.br/gaming"> <img src="https://www.logitechstore.com.br/media/wysiwyg/Base/svg/minilogo-logitechG.svg" alt=""/> </a> </div><div class="header-mobile__list-sep"> <a href="https://logitechstore.com.br/jaybird"> <img src="https://www.logitechstore.com.br/media/wysiwyg/Base/svg/minilogo-jaybird.svg" alt=""/> </a> </div><div class="header-mobile__list-sep"> <a href="https://logitechstore.com.br/ultimate-ears"> <img src="https://www.logitechstore.com.br/media/wysiwyg/Base/svg/minilogo-ultimate-ears.svg" alt=""/> </a> </div><div class="header-mobile__list-sep"> <a href="http://www.astrogamingstore.com.br"> <img src="https://www.logitechstore.com.br/media/wysiwyg/Base/svg/minilogo-astro.svg" alt=""/> </a> </div><div class="header-mobile__list-sep"> <a href="https://logitechstore.com.br/bluemic"> <img src="https://www.logitechstore.com.br/media/wysiwyg/Base/svg/minilogo-blue.svg" alt=""/> </a> </div></div></li>';
		$j('#header__submenu .ul--0 .li--0:last-child').after(marcas__mobile);
    
    	// Altera texto produto esgotado
    	$j('.product__soldoff').text("Avise-me quando este produto estiver disponivel");
    })
    .on('resizeStop', function(e){
        // Safe window.resize
	...
	.on('zoomCreated', function () {
        makeGallery();
    });


function makeGallery(){
    $j('.product-image-gallery #image-index-id , .product-image-gallery script , .product-image-container script').remove();
    $j('.product-image-gallery img').removeClass('gallery-image', 'zoom-available');
    $j('.product-image').removeClass('product-image-zoom', 'zoom-available');
    $j('.product-image-thumbs a').removeClass('thumb-link',' on');

    $j('.product-image-gallery img').each(function(index, value) {
        let $src = $j(this).attr('src'),
            $dsrc = $j(this).attr('data-src'),
            $attribute = $j(this).attr('id');

        if( this.id == 'image-main' ){
            $j('#'+$attribute).attr('data-flickity-lazyload-src', $src);
        }else{
            $j('#'+$attribute).attr('data-flickity-lazyload-src', $dsrc);
        }
    });

    var $productflick = $j('.product-image-gallery').flickity({
        lazyLoad: true,
        prevNextButtons: false,
    });

    var $productflicknav = $j('.more-views .product-image-thumbs').flickity({
    ...
    // Previous
    $j('.flickity-button.previous').on( 'click', function() {
      	$productflick.flickity('previous');        
    });
    
    // Next
    $j('.flickity-button.next').on( 'click', function() {
      	$productflick.flickity('next');             
    });
}

function banner__animation(e){       
	let name = $j('#home__banner__produtos .product__name');
}
...
const images = document.querySelector(".___carousel-").children;
